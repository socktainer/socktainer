...
struct EngineStateLockTests {
    ...
    func defaultStateIsOutsideHome() {
        let directory = SocktainerDirectories.engineStateDirectory(environment: [:])
        #expect(!directory.path.hasPrefix(SocktainerDirectories.hostHome.path + "/"))
    }

    @Test("engine state can be isolated from the exported host directory")
    func isolatedStateDirectory() {
        let directory = SocktainerDirectories.engineStateDirectory(
            environment: [
                "SOCKTAINER_HOST_HOME_DIRECTORY": "/exported",
                "SOCKTAINER_ENGINE_STATE_DIRECTORY": "/private/engine",
            ]
        )
