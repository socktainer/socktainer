    ...
    func connectsToGenerationScopedVsockPath() async throws {
        ...
        let expected = URL(fileURLWithPath: "/tmp", isDirectory: true)
            .appendingPathComponent(
                "socktainer-vmm-\(ready.generation.uuidString.lowercased())",
                isDirectory: true
            )
...
private struct Fixture {
    ...
    init() throws {
        let bindSource = FileManager.default.temporaryDirectory
            .appendingPathComponent("socktainer-runtime-machine-\(UUID().uuidString)", isDirectory: true)
        let state = FileManager.default.temporaryDirectory
            .appendingPathComponent("socktainer-engine-\(UUID().uuidString)", isDirectory: true)
