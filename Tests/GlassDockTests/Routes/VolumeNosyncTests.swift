...
struct VolumeNosyncTests {
    ...
    @Test("glassdockLabel constant matches expected key")
    func syncLabelKey() {
        #expect(Filesystem.SyncMode.glassdockLabel == "glassdock.volume.sync")
    }
    ...
    func dockerMetadataRoundTrip() {
        let stored = VolumeConfiguration(
            ...
            source: "/tmp/pgdata",
            labels: [
                Filesystem.SyncMode.glassdockLabel: "fsync",
                "purpose": "database",
            ],
            options: ["size": "10G"]
        )
        ...
        #expect(volume.Labels?[Filesystem.SyncMode.glassdockLabel] == nil)
    }
}
