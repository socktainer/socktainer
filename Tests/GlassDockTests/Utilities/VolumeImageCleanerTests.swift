...
final class VolumeImageCleanerTests {
    ...
    func optOut() {
        #expect(VolumeImageCleaner.isEnabled(labels: [:], environment: ["GLASSDOCK_CLEAN_VOLUMES": "false"]) == false)
        #expect(VolumeImageCleaner.isEnabled(labels: [:], environment: ["GLASSDOCK_CLEAN_VOLUMES": "FALSE"]) == false)
        #expect(VolumeImageCleaner.isEnabled(labels: ["glassdock.clean-volumes": "false"], environment: [:]) == false)
        #expect(VolumeImageCleaner.isEnabled(labels: [:], environment: [:]) == true)
        #expect(VolumeImageCleaner.isEnabled(labels: ["glassdock.clean-volumes": "true"], environment: [:]) == true)
    }
}
