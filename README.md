...
- `docker top` runs `ps` **inside the container** — Docker inspects processes from the host, which is not possible across the VM boundary. Images without a `ps` binary (scratch, distroless) get an explicit error.
...
- `docker save` currently fails: Apple's Containerization `ContentStore` keeps image metadata but not the underlying content blobs, so there is nothing to export ([platform limitation](https://github.com/apple/containerization)). This also breaks the `docker save | docker load` round-trip.
...
- Not yet implemented (endpoints answer an explicit error instead of pretending): `docker commit` and `docker diff`.
