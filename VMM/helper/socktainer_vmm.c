...
static void usage(const char *program) {
    fprintf(stderr,
        ...
        "--tcp-relay-socket PATH --console-log PATH --parent-pid PID "
...
static void fail_krun(const char *operation, int32_t result) {
    ...
    fprintf(stderr, "socktainer-vmm: %s failed: %s (%d)\n", operation,
...
static uint32_t parse_uint32(const char *name, const char *value, uint32_t minimum,
    ...
    if (errno != 0 || end == value || *end != '\0' || parsed < minimum || parsed > maximum) {
        fprintf(stderr, "socktainer-vmm: invalid %s: %s\n", name, value);
        exit(2);
    }
...
static void require_absolute(const char *name, const char *path) {
    if (path == NULL || path[0] != '/') {
        fprintf(stderr, "socktainer-vmm: %s must be an absolute path\n", name);
        exit(2);
    }
}
...
static struct configuration parse_configuration(int argc, char **argv) {
    ...
    if (configuration.parent_pid <= 1 || getppid() != configuration.parent_pid) {
        fprintf(stderr, "socktainer-vmm: parent process is not alive\n");
        exit(125);
    }
    if (access(configuration.kernel, R_OK) != 0 || access(configuration.root_disk, R_OK) != 0 ||
        ...
        fprintf(stderr, "socktainer-vmm: a required artifact or directory is inaccessible: %s\n",
            strerror(errno));
        exit(125);
    }
...
int main(int argc, char **argv) {
    ...
    if (setpgid(0, 0) != 0) {
        fprintf(stderr, "socktainer-vmm: create process group failed: %s\n", strerror(errno));
        return 125;
    }
    struct socktainer_gvproxy gvproxy = {.pid = -1};
    if (socktainer_gvproxy_start(argv[0], configuration.console_log, &gvproxy) != 0) {
        fprintf(stderr, "socktainer-vmm: start gvproxy failed: %s\n", strerror(errno));
        return 125;
    }
    if (socktainer_gvproxy_watch_parent(configuration.parent_pid) != 0) {
        fprintf(stderr, "socktainer-vmm: supervise parent failed: %s\n", strerror(errno));
        socktainer_gvproxy_stop(&gvproxy);
        return 125;
    }
    if (!write_network_state(configuration.console_log)) {
        fprintf(stderr, "socktainer-vmm: publish network state failed: %s\n",
            strerror(errno));
        socktainer_gvproxy_stop(&gvproxy);
        return 125;
    }
    ...
    fail_krun("configure bind filesystem", krun_add_virtiofs4((uint32_t)context,
        "socktainer-home", configuration.bind_source, 0, false,
    ...
    fail_krun("configure TCP relay port", krun_add_vsock_port2((uint32_t)context, 1026,
        configuration.tcp_relay_socket, true));
    ...
    const char *guest_environment[] = {
        ...
        NULL,
        NULL,
        "SOCKTAINER_GUEST_DNS=192.168.127.1",
        NULL,
    };
    ...
    snprintf(guest_address_environment, sizeof(guest_address_environment),
        "SOCKTAINER_GUEST_ADDRESS=192.168.127.2/24");
    snprintf(guest_gateway_environment, sizeof(guest_gateway_environment),
        "SOCKTAINER_GUEST_GATEWAY=192.168.127.1");
