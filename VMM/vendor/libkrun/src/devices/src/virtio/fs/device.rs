...
impl Fs {
    pub fn new(
        ...
        let host_mutable = matches!(semantics, PermissionSemantics::LinuxSimplified);
        let attr_timeout = if host_mutable {
            // The host can modify shared files without going through FUSE.
        ...
        let fs_cfg = shared_dir.map(|root_dir| passthrough::Config {
            ...
            semantics,
            attr_timeout,
            entry_timeout: if host_mutable {
                Duration::from_secs(0)
            } else {
                Duration::from_secs(5)
            },
            cache_policy: if host_mutable {
                passthrough::CachePolicy::Never
            } else {
                passthrough::CachePolicy::Auto
            },
            ..Default::default()
        });
