...
pub fn build_microvm(
    ...
    #[cfg(target_os = "macos")]
    let memory_reclaimer = vm.memory_reclaimer(guest_memory.clone());
...
fn create_vcpus_aarch64(
    ...
    nested_enabled: bool,
    memory_reclaimer: Arc<crate::vstate::MemoryReclaimer>,
) -> super::Result<Vec<Vcpu>> {
    let mut vcpus = Vec::with_capacity(vcpu_config.vcpu_count as usize);
    ...
    for cpu_index in 0..vcpu_config.vcpu_count {
        ...
        vcpu.set_memory_reclaimer(memory_reclaimer.clone());
...
fn attach_balloon_device(
    ...
    #[cfg(target_os = "macos")]
    let balloon = devices::virtio::Balloon::new_with_reclaimer(Some(vmm.memory_reclaimer.clone()));
    #[cfg(not(target_os = "macos"))]
    let balloon = devices::virtio::Balloon::new();
    let balloon = Arc::new(Mutex::new(balloon.unwrap()));
