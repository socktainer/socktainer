...
struct VolumeCreateRoute: RouteCollection {
    ...
    func handler(_ req: Request) async throws -> Response {
        ...
        // Glass Dock-specific) and persist it as a label so ContainerCreateRoute
        ...
        guard
            !originalLabels.keys.contains(where: {
                LabelNormalization.sanitizeKey($0) == Filesystem.SyncMode.glassdockLabel
            })
        else {
            throw Abort(
                .badRequest,
                reason: "Label key '\(Filesystem.SyncMode.glassdockLabel)' is reserved for internal use"
            )
        }
        ...
        if let syncValue = driverOpts.removeValue(forKey: "sync") {
            ...
            labels[Filesystem.SyncMode.glassdockLabel] = syncValue
        }
        if isAnonymous {
        ...
        do {
            ...
            // glassdock's create throws "already exists", which breaks tools
