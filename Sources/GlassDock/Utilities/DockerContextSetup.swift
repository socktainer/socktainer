...
/// Creates (or updates) a Docker context named "glassdock" pointing at the
/// local Unix socket, so users can run `docker context use glassdock` instead
...
enum DockerContextSetup {
    static let contextName = "glassdock"
    ...
    private static let log = Logger(label: "glassdock.context")

    static func install(homeDirectory: String) {
        let socketPath = "\(homeDirectory)/.glassdock/container.sock"
        ...
        let payload: [String: Any] = [
            "Name": contextName,
            "Metadata": ["Description": "Glass Dock — Docker API over Apple Container"],
            "Endpoints": [
                "docker": [
