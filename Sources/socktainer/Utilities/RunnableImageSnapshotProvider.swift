...
struct LiveRunnableImageSnapshotProvider: RunnableImageSnapshotProviding {
    ...
    private func exactSnapshot(
        ...
        let exactImage = Containerization.Image(
            description: .init(
                reference: "socktainer-exact@\(variant.descriptor.digest)",
...
struct PreparedContainerRootFS: Sendable {
    static let ownershipMarkerFilename = ".socktainer-create-owned"
...
struct ContainerRootFSMaterializer: Sendable {
    static let stagingBundlePrefix = ".socktainer-create-staging-"
    ...
    /// Removes only a valid Socktainer marker for the exact stale attempt. The
