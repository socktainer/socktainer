...
enum FilteredStreamDecoder {
    ...
    enum Compression {
        ...
        fileprivate var code: socktainer_filtered_stream_codec {
            switch self {
            case .gzip: SOCKTAINER_FILTER_GZIP
            case .bzip2: SOCKTAINER_FILTER_BZIP2
            case .xz: SOCKTAINER_FILTER_XZ
            }
        }
    }
    ...
    static func decompress(
        ...
        guard
            let stream = socktainer_filtered_stream_open(
        ...
        defer { socktainer_filtered_stream_close(stream) }
        ...
        while true {
            ...
            let bytesRead = buffer.withUnsafeMutableBytes { bytes in
                socktainer_filtered_stream_read(
                    stream,
                    bytes.baseAddress,
                    bytes.count
                )
            ...
            guard bytesRead >= 0 else {
                if socktainer_filtered_stream_last_error(stream)
